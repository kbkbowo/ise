...
class GoalGaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, x_cond, task_embed, supp, suc, clip_denoised=False, guidance_weight=0):
        preds = self.model_predictions(x, t, x_cond, task_embed, supp, suc, guidance_weight=guidance_weight)
    ...
    @torch.no_grad()
    def p_sample(self, x, t: int, x_cond, task_embed, supp, suc, guidance_weight=0):
        b, *_, device = *x.shape, x.device
        batched_times = torch.full((b,), t, device = x.device, dtype = torch.long)
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(x, batched_times, x_cond, task_embed, supp, suc, clip_denoised = True, guidance_weight=guidance_weight)
        noise = torch.randn_like(x) if t > 0 else 0. # no noise if t == 0
        pred_img = model_mean + (0.5 * model_log_variance).exp() * noise
        return pred_img, x_start

    @torch.no_grad()
    def p_sample_loop(self, shape, x_cond, task_embed, supp, suc, return_all_timesteps=False, guidance_weight=0):
    ...
    @torch.no_grad()
    def ddim_sample(self, shape, x_cond, task_embed, supp, suc, return_all_timesteps=False, guidance_weight=0):
    ...
    @torch.no_grad()
    def sample(self, x_cond, task_embed, supp, suc, batch_size = 16, return_all_timesteps = False, guidance_weight=0):
        image_size, channels = self.image_size, self.channels
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn((batch_size, channels, image_size[0], image_size[1]), x_cond, task_embed, supp, suc, return_all_timesteps = return_all_timesteps, guidance_weight=guidance_weight)
    ...
    def p_losses(self, x_start, t, x_cond, task_embed, supp, suc, noise=None):
        ...
        model_out = self.model(torch.cat([x, x_cond], dim=1), t, task_embed, supp, suc)
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    x, x_cond, goal, supp, suc = next(self.dl)
                    x, x_cond, supp, suc = x.to(device), x_cond.to(device), supp.to(device), suc.to(device)
                    ...
                    with self.accelerator.autocast():
                        loss = self.model(x, x_cond, goal_embed, supp, suc)
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        with torch.no_grad():
                            ...
                            supps = []
                            sucs = []
                            for i, (x, x_cond, label, supp, suc) in enumerate(self.valid_dl):
                                xs.append(x)
                                x_conds.append(x_cond.to(device))
                                task_embeds.append(self.encode_batch_text(label))
                                supps.append(supp.to(device))
                                sucs.append(suc.to(device))
                            
                            with self.accelerator.autocast():
                                all_xs_list = list(map(lambda n, c, e, s, sc: self.ema.ema_model.sample(batch_size=n, x_cond=c, task_embed=e, supp=s, suc=sc), batches, x_conds, task_embeds, supps, sucs))
                        ...
                        utils.save_image(pred_img, str(self.results_folder / f'imgs/outputs/{milestone}.png'), nrow=n_rows+2)
